pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/calcuagvtwo"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-AgCalV2-WebApp-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop calcuagvtwo | true"
                sh "docker rm calcuagvtwo | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name calcuagvtwo -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service calcuagvtwo-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"calcuagvtwo\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
